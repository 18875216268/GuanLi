...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (databaseMenu) {
        databaseMenu.innerHTML = `
                <a href="#" onclick="showAddDatabaseModal()">添加数据库</a>
                <a href="#" onclick="showDeleteDatabaseModal()">删除数据库</a>
                <a href="#" onclick="showDatabaseModal()">表字段管理</a>
            `;
    }
});
...
function showDatabaseMenu(event) {
    ...
    // 切换菜单显示状态
    if (databaseMenu.style.display === 'block') {
        ...
        // 添加点击外部关闭菜单的事件监听
        setTimeout(() => {
            document.addEventListener('click', closeDatabaseMenu);
        }, 0);
    }
}

// 关闭数据库操作菜单
function closeDatabaseMenu(event) {
    const databaseMenu = document.getElementById('databaseMenu');
    const databaseBlock = document.getElementById('数据库块');

    if (!databaseMenu.contains(event.target)) {
        databaseMenu.style.display = 'none';
        document.removeEventListener('click', closeDatabaseMenu);
    }
}
...
function closeAllMenus() {
    ...
    // 关闭批量操作菜单
    const batchMenu = document.querySelector('.batch-operations-menu');
    if (batchMenu) {
        batchMenu.remove();
    }
}
...
function showAddDatabaseModal() {
    ...
    document.getElementById('dbConfig').value = '';
}
...
async function addDatabase() {
    ...
    try {
        ...
        if (Object.values(configs).some(value => value.name === dbName)) {
            showToast('数据库名称已存在！', 'error');
            return;
        }
...
async function confirmDeleteDatabases() {
    const selectedOptions = document.querySelectorAll('.数据库选项.selected');

    if (selectedOptions.length === 0) {
        showToast('请选择要删除的数据库！', 'warning');
        return;
    }

    if (!confirm(`确定要删除选中的 ${selectedOptions.length} 个数据库配置吗？`)) {
        return;
    }

    try {
        const deletePromises = Array.from(selectedOptions).map(option => {
            const key = option.getAttribute('data-key');
            return configDB.ref('database').child(key).remove();
        });

        await Promise.all(deletePromises);
        showToast('数据库删除成功！', 'success');
        closeDeleteDatabaseModal();
    } catch (error) {
        console.error('删除数据库失败：', error);
        showToast('删除数据库失败，请重试！', 'error');
    }
}
